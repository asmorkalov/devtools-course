...
{
	a[i] * fromTemperature.value + b[i];
	// a = 1; b = 0;
	// a = 1; b = -273;
	int a[]
}

Temperature ConvertFromCelsius(Temperature fromTemperature, TemperatureUnit toUnit);

Temperature TemperatureConvertor::ConvertTo(Temperature fromTemperature, TemperatureUnit toUnit)
{
	Temperature out = ConvertToCelsius(fromTemperature);
	out = ConvertFromCelsius(out, toUnit);

	if ((fromTemperature.unit
	 == Celsius) && (toUnit == Kelvin))
	{
